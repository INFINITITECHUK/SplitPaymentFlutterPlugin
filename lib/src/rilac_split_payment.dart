...
class RilacSplitPayment extends Functions {
  ...
  RilacSplitPayment.config({required String apiBaseUrl, required String basicToken, required String apiModuleKey}) {
    ...
    {
      if(value.isEmpty){
        SharedPrefs.saveConfig(baseUrl: apiBaseUrl,
            module: apiModuleKey,
            token: basicToken),
      }
    });
    ...
    try {
      ...
      if (amount <= 0 || splitNumber <= 0) {
        return {
          ...
          "statusCode": 400,
          "payload": null,
          "message": "Amount and split number must be greater than zero",
        };
      }
      ...
      return {
        ...
        "statusCode": 500,
        "payload": null,
        "message": "Failed to setup split payment: ${e.toString()}",
      };
    }
  }
    ...
    try {
      ...
      if (splitCustomerDetails.length != splitNumber) {
        return {
          ...
          "statusCode": 400,
          "payload": null,
          "message": "The list length must be equal to the split number.",
        };
      }
      ...
      if (totalSplitAmount != amount) {
        return {
          ...
          "statusCode": 400,
          "payload": null,
          "message": "Total split amount (${totalSplitAmount.toStringAsFixed(2)}) must equal the original amount (${amount.toStringAsFixed(2)}).",
        };
      }
      ...
      return {
        ...
        "statusCode": 200,
        "payload": jsonPayload,
        "message": "Customer split payment setup successful.(status: 0 = pending, 1 = processing, 2 = complete, 3 = failed)",
      };
    } catch (e) {
      return {
        "isSuccess": false,
  ...
  Future<Map<String, dynamic>> updateSingleCustomerInfo({required int customerId, required SplitPaymentCustomerDetailsModel splitCustomerDetails}) async{
    try {
      ...
      return {
        ...
        "statusCode": 200,
        "payload": jsonPayload,
        "message": "Customer details update successful",
      };
    } catch (e) {
      return {
        "isSuccess": false,
        "statusCode": 500,
        "payload": null,
        "message": "Failed to update customer details: ${e.toString()}",
      };
    }
  }
}